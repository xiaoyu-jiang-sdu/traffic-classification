...
def sample_pcap(split_pcap_dir, output_dir='flows_sampled', maximum=5000):
...
if __name__ == '__main__':
    ...
    path = "E:/ChromeDownload/USTC-TFC2016-master"
    output_dir = os.path.join(path, "test")
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    ...
    for dirname in dirs:
        dir_path = os.path.join(path, 'split',  dirname)
        if os.path.isdir(dir_path):
            sample_test(dir_path, os.path.join(output_dir, dirname))